...
def telegram_chat_targets_for_moderation_recipient(owner_id: int, user) -> list[str]:
    ...
    Профиль и карточка «Команда → доступы» синхронизируются при сохранении; на практике значения совпадают.
    Объединяем оба источника на случай старых данных. Порядок: владелец — сначала профиль, затем команда;
