...
def process_telegram_update_core(bot_id: int, update_data: dict) -> None:
    """
    Синхронная обработка одного Telegram update (PTB + asyncio.run).

    Вызывается из HTTP webhook по умолчанию, чтобы бот отвечал без Celery-воркера.
    """
    from bots.models import SuggestionBot
    from bots.telegram.handlers import build_application
    from telegram import Update

    logger.info('[TG] update bot_id=%s keys=%s', bot_id, list(update_data.keys())[:20])
    ...
    asyncio.run(process())
    logger.info('[TG] processed update ok bot_id=%s', bot_id)
...
def process_telegram_update_task(self, bot_id: int, update_data: dict):
    """
    Фоновая обработка update (если включён TELEGRAM_WEBHOOK_USE_CELERY).
    Иначе вебхук вызывает process_telegram_update_core напрямую.
    """
    try:
        process_telegram_update_core(bot_id, update_data)
        return True
    except Exception as e:
