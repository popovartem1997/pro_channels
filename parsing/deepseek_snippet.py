...
def rewrite_for_feed_post(
    *,
    ...
    tone — ключ настроения из AI_POST_MOODS (кнопки в ленте).
    """
    tone_key = normalize_ai_tone(tone)
    tone_rule = _MOOD_INSTRUCTIONS.get(tone_key, _MOOD_INSTRUCTIONS[DEFAULT_AI_TONE])
    ...
    user_msg = (
        ...
        f'Выбранное настроение поста: «{tone_key}» — весь текст (заголовок и тело) должен ему соответствовать.'
    )
