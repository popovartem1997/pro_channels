...
def _telethon_session_lock(owner_id: int):
    """
    Один воркер Celery на SQLite-сессию Telethon (owner): иначе database is locked
    ...
    lock = r.lock(
        f'pch:telethon:owner:{int(owner_id)}',
        timeout=900,
    ...
    try:
        ...
        if not acquired:
            raise RuntimeError(
                'Парсинг Telegram: не удалось занять сессию за 120 с '
                '(вероятно, параллельно идёт другой парсинг этого же аккаунта).'
            )
        yield
    finally:
        if acquired:
