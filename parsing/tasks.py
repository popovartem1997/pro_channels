...
def _telethon_session_lock(
    ...
    Ожидание lock в Redis делается короткими попытками (до TELETHON_REDIS_LOCK_WAIT суммарно),
    чтобы импорт истории мог писать в журнал (on_lock_wait_tick(elapsed_sec)) вместо «тишины» 10 минут.
    """
    from django.conf import settings
    ...
    # Суммарное ожидание освобождения (сек.); было 120 — часто не хватало при фоновом импорте.
    wait = float(getattr(settings, 'TELETHON_REDIS_LOCK_WAIT', 600))
    # Одна попытка acquire в Redis — не больше (сек.), иначе нет точек для on_lock_wait_tick.
